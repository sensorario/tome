...
function Tomato() {
    return (
        ...
        >
            ...
            <path
                d="M10 5.5 C10 5.5 9 3 7 3 C7 5 8.5 5.5 10 5.5Z"
                fill="#38a169"
            />
            <path
                d="M10 5.5 C10 5.5 11 3 13 3 C13 5 11.5 5.5 10 5.5Z"
                fill="#48bb78"
            />
            <path
                d="M10 5.5 L10 7.5"
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    {ws.tasks_expired > 0 && (
                                        ...
                                        >
                                            {ws.tasks_expired} scadut
                                            {ws.tasks_expired === 1 ? "o" : "i"}
                                        </span>
                                    )}
                                </div>
                            </div>
...
export default function App() {
    ...
    const fetchTasks = async (tok) => {
        try {
            ...
            setTasks(
                (data["simplanner-tasks"] ?? []).filter((t) => !t.archived)
            );
        } catch {
    ...
    const load = async (tok) => {
        ...
        try {
            const res = await fetch(
                "https://api.simonegentili.com/tome/timeboxes",
                {
                    headers: { Authorization: `Bearer ${tok}` },
                }
            );
    ...
    const repeatableIds = new Set(
        Object.values(
            (groups[todayKey] ?? []).reduce((acc, p) => {
                ...
                if (
                    !acc[key] ||
                    parseUTC(p.started_at) > parseUTC(acc[key].started_at)
                ) {
                    acc[key] = p;
                }
                return acc;
        ...
        <>
            ...
            >
                    ...
                    >
                        {workspaces.length > 0 && (
                            <Button
                                ...
                                label={
                                    workspaces.find((w) => w.current)?.name ??
                                    "Workspace"
                                }
                                style={{
                                    fontSize: "0.9rem",
                ...
                {!token && (
                    ...
                    >
                        Accedi per visualizzare i timebox e iniziare a
                        tracciarli.
                    </p>
                )}
                    ...
                    <div style={{ marginTop: "1.5rem" }}>
                        ...
                        >
                            ...
                            {filteredItems.length === 0 && (
                                ...
                                >
                                    Nessun timebox nel periodo selezionato
                                </li>
                            )}
                        </ul>
                    </div>
